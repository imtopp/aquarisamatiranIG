...
def _init_ctx(monkeypatch, tmp_path: Path, overrides: dict = None):
    data = {**INITIAL, **(overrides or {})}
    ctx = AccountContext(name="test", base=tmp_path, enabled=True, config_data={})
    ctx.source_of_truth.write_text(json.dumps(data, indent=2), encoding="utf-8")
    monkeypatch.setattr(mgr, "get_account", lambda account=None: ctx)
    return ctx


class TestTelegramAddCategory:
    def test_adds_new_category(self, monkeypatch, tmp_path):
        ctx = _init_ctx(monkeypatch, tmp_path)
...
class TestTelegramMoveTopic:
    def _init_ctx_two_cats(self, monkeypatch, tmp_path):
        data = dict(INITIAL)
        data["categories"]["2"] = {
            "title": "Category 2",
            "subtitle": "",
    ...
    def test_move_topic(self, monkeypatch, tmp_path):
        ctx = self._init_ctx_two_cats(monkeypatch, tmp_path)
